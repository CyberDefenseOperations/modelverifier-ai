...
- **CSA AICM** (`aicm`) covers all 6 layers with 45 total mappings against AICM v1.1 (247 objectives, 18 domains; verified against the authoritative spreadsheet generated 2026-06-18). The AICM v1.1 uses official domain abbreviations: A&A, AIS, BCR, CCC, CEK, DCS, DSP, GRC, HRS, IAM, IPY, I&S, LOG, MDS, SEF, STA, TVM, UEM. This matrix uses 10 of those domains (A&A, AIS, CCC, DSP, GRC, LOG, MDS, SEF, STA, TVM). All 45 mappings carry `mapping_confidence: medium` + `provisional: true` because translating model-assurance controls to cloud security controls involves interpretive judgment. A CSA assessor review is recommended before claiming high confidence. See CONTRIBUTING.md section 4.7 for the full domain table.
